"""Add standards / curriculum alignment tagging (Task #48)
...
Two new tables:
  - standards: the school's curriculum taxonomy (School Admin managed).
  - content_standard_tags: links a Standard to a Lesson OR an Assignment
    (exactly one of lesson_id/assignment_id set -- enforced in the API
    router, not a DB constraint, matching this app's existing convention).
"""
...
def upgrade() -> None:
    ...
    if 'content_standard_tags' not in existing_tables:
        ...
        op.create_index('ix_content_standard_tags_standard', 'content_standard_tags', ['standard_id'])
        op.create_index('ix_content_standard_tags_lesson', 'content_standard_tags', ['lesson_id'])
        op.create_index('ix_content_standard_tags_assignment', 'content_standard_tags', ['assignment_id'])
