...
export default function GradesPage() {
  ...
  useEffect(() => {
    schoolAdminAPI.getGrades().then((res) => {
      if (res.data && Array.isArray(res.data)) {
        const fetched = res.data.map((g) => ({
          id: g.id,
          name: g.name,
          sections: g.sections || 1,
          studentsCount: g.students_count || 0,
          teachersCount: g.teachers_count || 0,
          averagePerformance: g.average_performance || 0,
        }));
        setGrades(fetched);
      }
      setLoading(false);
  ...
  const stats = useMemo(() => ({
    ...
    totalStudents: grades.reduce((sum, g) => sum + (g.studentsCount || 0), 0),
    totalSections: grades.reduce((sum, g) => sum + (g.sections || 1), 0),
  }), [grades]);
  ...
  function handleSave(data) {
    if (editingGrade) {
      ...
      schoolAdminAPI.createGrade({ name: data.name, code: data.code || data.name }).then((res) => {
        const created = res.data;
        setGrades((prev) => [{ id: created.id, name: created.name, sections: 1, studentsCount: 0, teachersCount: 0, averagePerformance: 0 }, ...prev]);
      }).catch(() => {
        setGrades((prev) => [{ id: Date.now().toString(), studentsCount: 0, teachersCount: 0, averagePerformance: 0, ...data }, ...prev]);
      });
    }
    setFormOpen(false);
  }
  ...
  function confirmDelete() {
    setDeleting(true);
    schoolAdminAPI.deleteGrade(deleteTarget.id).catch(() => {});
