...
import { announcementsAPI, schoolAdminAPI } from "../../services/api";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          A school-wide view of announcements, discussions, and creative work students have shared.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white">
        ...
        {announcementsLoading ? (
          ...
          <EmptyState title="No announcements yet" description="Post your first school-wide or course announcement." icon={Megaphone} />
        ) : (
      ...
      {/* Discussions -- Admin's first entry point into the existing (already tenant-scoped)
          discussions API: backend PARTICIPANT_ROLES already includes ADMIN, this hub page
          is simply the first UI surface for it. */}
      <div>
        <h2 className="mb-3 text-base font-semibold text-slate-900">Discussions & Doubts</h2>
        {coursesLoading ? (
          <div className="rounded-xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-400">Loading courses...</div>
        ) : courses.length === 0 ? (
          <div className="rounded-xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-500">
            No courses in your school yet.
          </div>
        ) : (
