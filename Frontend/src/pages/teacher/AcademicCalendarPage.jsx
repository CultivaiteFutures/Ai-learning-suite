...
import { calendarAPI, teacherAPI } from "../../services/api";
...
export default function AcademicCalendarPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    Promise.all([calendarAPI.listEvents(), teacherAPI.getCourses()])
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Schedule exams, deadlines, and events for your courses.</p>
        </div>
        <CalendarSyncButton />
      </div>
