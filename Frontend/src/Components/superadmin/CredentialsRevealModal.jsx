  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="School Onboarded Successfully" maxWidth="max-w-md">
      ...
      <div className="mt-5 space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm">
        ...
        <div className="flex justify-between"><span className="text-slate-500">Admin Password</span><span className="font-mono font-medium text-slate-800">{admin.tempPassword}</span></div>
        {school.aiProvider && (
          <div className="flex justify-between"><span className="text-slate-500">AI Provider</span><span className="font-medium text-indigo-700 capitalize">{school.aiProvider}</span></div>
        )}
      </div>
