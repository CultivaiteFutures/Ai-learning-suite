...
export default function TeacherDashboard() {
  ...
  const pendingGrading = assignments.filter((a) => a.status === "grading" || a.submissionsCount < a.totalStudents).length;
  const recentCourses = [...courses].sort((a, b) => new Date(b.createdDate) - new Date(a.createdDate)).slice(0, 5);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <DashboardCard label="Students" value={mockTeacherStudents.length} icon={Users} accent="rose" />
      </div>
              ...
              upcomingAssignments.map((a) => (
                <div key={a.id} className="flex items-center justify-between px-5 py-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {courseNameById[a.courseId] || "Unknown course"} · Due {a.dueDate}
                    </p>
                  </div>
