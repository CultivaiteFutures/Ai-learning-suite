...
const SUBJECT_OPTIONS = [
  "Mathematics", "Science", "English", "History", "Computer Science", "Art", "Music", "Physical Education",
...
export default function CourseBuilderPage() {
  ...
  async function handleGenerateFromPDF(file) {
    setPdfLoading(true);
    try {
      const generated = await generateCourseFromPDF(file?.name || "uploaded-document.pdf", {
        grade: course.grade,
        subject: course.subject,
      });
          ...
          {(course.modules || []).map((module, mi) => (
            <div key={module.id} className="rounded-xl border border-slate-200 bg-white p-5">
              <p className="text-sm font-semibold text-slate-900">
                Module {mi + 1}: {module.name.replace(/^Module \d+:\s*/, "")}
              </p>
